...
SelectVO = function(keys)
	...
	if selectedstr ~= nil and selectedid2 ~= nil then
		...
		if votimer[keys.PlayerID] ~= nil then
			if GameRules:GetGameTime() - votimer[keys.PlayerID] > chat_cooldown + vousedcol[keys.PlayerID] and (phraseDoesntHasCooldown == nil or phraseDoesntHasCooldown == true) then
...
function ChatSound(phrase, playerId)
	for playerId = 0, 233 do
		if PlayerResource:IsValidTeamPlayerID(playerId) and not PlayerResource:IsFakeClient(playerId) then
			EmitAnnouncerSoundForPlayer(phrase, playerId)
		end
	end
end
